...
def eliminados(grupoA,grupoB,grupoC):
    ...
    for index in grupoA:
        if index["Inasistencias"] >= 15: #grupoA[index]["Inasistencias"]
            contA += 1
            eliminadosA.append(grupoA[index]["Inasistencias"])
            grupoA.pop(index)

    for index in grupoB:
        if index["Inasistencias"] >= 15:
            contB += 1
...
"""def ceremonia(grupoA,grupoB,grupoC):
    ...
    if talA>talB:
        if talA>talC:
            ...
            print(f"El coder que fue más veces monitor es {mxTalA}")
  """      
def repetir(grupo, nombre, l1, l2, l3, l4, l5):
    # Valores
    mon = []
    tal = []
    col = []
    ...
    for index in grupo:
        if grupo[index][l1] > numMon:
            mon.append(grupo[index][l1])
            maxMon.append(grupo[index][nombre])
            numMon = grupo[index][l1]

        if grupo[index][l2] > numTal:
            tal.append(grupo[index][l2])
            maxTal.append(grupo[index][nombre])
            numTal = grupoA[index][l2]

        if grupo[index][l3] > numCol:
            col.append(grupo[index][l3])
            maxCol.append(grupo[index][nombre])
            numCol = grupo[index][l3]      

        if grupo[index][l4] > numPar:
            par.append(grupo[index][l4])
            maxPar.append(grupo[index][nombre])
            numPar = grupo[index][l4]  

        if grupo[index][l5] > numNil:
            nil.append(grupo[index][l5])
            maxNil.append(grupo[index][nombre])
            numNil = grupo[index][l5]

    print(f"El coder del grupo A que ha sido más veces monitor es {maxMon}")  

    nombreValor = { maxMon : mon, maxTal : tal, maxCol : col, maxPar : par, maxNil : nil }
